...
class HelpCommand(vbot.HelpCommand):
    async def send_bot_help(self, mapping):
        ...
        msg = f"""```yaml
> Vbot by {self.context.bot.get_user(main.__author_id__)} | {main.__author_id__} <
> Siphon by {self.context.bot.get_user(main.__extendor_id__)} | {main.__extendor_id__} <
...
!# The category name must be case-sensitive. Example: "{self.context.clean_prefix}help Raiding"```"""
        await self.context.message.edit(content=msg)
...
!# Run {self.context.clean_prefix}{self.invoked_with} [command] to learn more about a command```"""
    ...
    async def send_error_message(self, error: str):
        await self.context.message.edit(error)
